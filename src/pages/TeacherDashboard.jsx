...
function OverviewTab({ user, setActiveTab }) {
  ...
  return (
    <div style={{ animation:"fadeIn 0.3s ease" }}>
      {/* Welcome banner */}
      <div style={{ background:"linear-gradient(135deg,#f59e0b,#d97706)", borderRadius:20, padding:"24px 28px",
        marginBottom:24, color:"white", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <div>
          <h1 style={{ fontSize:22, fontWeight:900, margin:"0 0 6px", letterSpacing:"-0.3px" }}>
            Good morning, {user.name?.split(" ")[0]}! 👋
          </h1>
          <p style={{ fontSize:13, margin:0, opacity:0.88 }}>
            {user.subject} Teacher · {user.batch || "SpacECE"} · {new Date().toLocaleDateString("en-IN",{weekday:"long",day:"numeric",month:"long"})}
          </p>
        </div>
          ...
          <div style={{ display:"flex", alignItems:"flex-end", gap:8, height:110 }}>
            {MOCK_ATTENDANCE_MONTHLY.map((d,i)=>(
              <div key={i} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:4 }}>
                <span style={{ fontSize:9, fontWeight:700, color: d.val>=90?"#10b981":d.val>=80?"#f59e0b":"#ef4444" }}>{d.val}%</span>
                <div style={{ width:"100%", borderRadius:"4px 4px 0 0",
                  background:d.val>=90?"#10b981":d.val>=80?"#f59e0b":"#ef4444",
                  height:`${d.val}%`, opacity:0.85 }}/>
                <span style={{ fontSize:9, color:"#9ca3af", fontWeight:600 }}>{d.month}</span>
              </div>
            ))}
          </div>
        </SectionCard>
          ...
          {MOCK_ASSIGNMENTS.map((a,i)=>(
            <div key={i} style={{ display:"flex", alignItems:"center", gap:10, padding:"8px 0", borderBottom:"1px solid #f3f4f6" }}>
              <div style={{ width:8, height:8, borderRadius:"50%", flexShrink:0,
                background:a.status==="approved"?"#10b981":a.status==="revision"?"#ef4444":"#f59e0b" }}/>
              <div style={{ flex:1 }}>
  ...
  return (
    <div style={{ animation:"fadeIn 0.3s ease" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
        {MOCK_COURSES.map((c,i)=>(
          <div key={i} style={{ background:"white", borderRadius:16, padding:"22px 24px",
            border:"1px solid #f1f5f9", boxShadow:"0 2px 8px rgba(0,0,0,0.04)",
            borderLeft:`4px solid ${c.status==="active"?"#f59e0b":"#3b82f6"}` }}>
        ...
        {MOCK_SCHEDULE.map((cl,i)=>{
          ...
          return (
            <div key={i} style={{ display:"flex", alignItems:"center", gap:16, padding:"14px 16px",
              background:bg[cl.status], borderRadius:10, marginBottom:8, border:"1px solid #f3f4f6" }}>
  ...
  return (
    <div style={{ animation:"fadeIn 0.3s ease" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))", gap:16 }}>
        {MOCK_GRADES.map((g,i)=>(
          <div key={i} style={{ background:"white", borderRadius:16, padding:"20px",
            border:"1px solid #f1f5f9", boxShadow:"0 2px 8px rgba(0,0,0,0.04)",
            borderTop:`3px solid ${g.avg>=85?"#10b981":g.avg>=75?"#f59e0b":"#ef4444"}` }}>
            ...
            <div style={{ height:8, background:"#f3f4f6", borderRadius:4, overflow:"hidden", marginBottom:12 }}>
              <div style={{ height:"100%", width:`${g.avg}%`, borderRadius:4,
                background:g.avg>=85?"#10b981":g.avg>=75?"#f59e0b":"#ef4444" }}/>
            </div>
            ...
            <div style={{ fontSize:11, color:"#6b7280", paddingTop:10, borderTop:"1px solid #f3f4f6" }}>
              📝 Assignments: {g.completed}/{g.assignments} submitted
            </div>
          </div>
        ))}
      </div>
    ...
    <div style={{ animation:"fadeIn 0.3s ease" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
        {MOCK_ASSIGNMENTS.map(a=>(
          <div key={a.id} style={{ background:"white", borderRadius:14, padding:"16px 20px",
            border:"1px solid #f1f5f9", boxShadow:"0 1px 4px rgba(0,0,0,0.04)",
            borderLeft:`4px solid ${a.status==="approved"?"#10b981":a.status==="revision"?"#ef4444":"#f59e0b"}` }}>
            <div style={{ display:"flex", alignItems:"center", gap:12 }}>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                ...
                {(a.status==="revision"||a.status==="pending") &&
                  <button onClick={()=>setUploadModal(true)} style={{ ...S.primaryBtn, padding:"6px 12px", fontSize:12 }}>
                    {a.status==="revision"?"Resubmit":"Submit"}
                  </button>
                }
              </div>
            </div>
            {a.status==="revision" && (
              <div style={{ marginTop:10, padding:"8px 12px", background:"#fef2f2", borderRadius:8, fontSize:12, color:"#991b1b" }}>
                ⚠️ Revision required — please review admin feedback and resubmit.
              </div>
            )}
          </div>
        ))}
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.45)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:999, backdropFilter:"blur(4px)" }}>
          <div style={{ background:"white", borderRadius:20, padding:"28px", width:"100%", maxWidth:480, boxShadow:"0 20px 60px rgba(0,0,0,0.2)" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
              ...
              <button onClick={()=>setUploadModal(false)} style={{ background:"none", border:"none", fontSize:20, cursor:"pointer", color:"#9ca3af" }}>✕</button>
            </div>
            ...
            <div style={{ border:"2px dashed #fbbf24", borderRadius:12, padding:"28px", textAlign:"center", marginBottom:16, background:"#fffbeb" }}>
              <div style={{ fontSize:32, marginBottom:8 }}>📎</div>
              <div style={{ fontSize:13, fontWeight:600, color:"#92400e" }}>Click to upload or drag & drop</div>
            ...
            <button onClick={()=>setUploadModal(false)} style={{ ...S.primaryBtn, width:"100%" }}>📤 Submit Assignment</button>
          </div>
        </div>
      )}
    </div>
    ...
    <div style={{ animation:"fadeIn 0.3s ease" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(340px,1fr))", gap:16 }}>
        {MOCK_CERTIFICATES.map((c,i)=>(
          <div key={i} style={{ background:"linear-gradient(135deg,#fffbeb,#fef3c7)", borderRadius:20,
            padding:"28px 24px", border:"2px solid #fbbf24", position:"relative", overflow:"hidden",
            ...
            <div style={{ fontSize:11, color:"#9ca3af", marginBottom:16 }}>
              🔑 Credential ID: <span style={{ fontWeight:700, color:"#374151" }}>{c.credentialId}</span>
            </div>
        ...
        {/* Locked certificate */}
        <div style={{ background:"#f9fafb", borderRadius:20, padding:"28px 24px",
          border:"2px dashed #e5e7eb", opacity:0.7 }}>
...
function ProfileTab({ user }) {
  ...
  const [form, setForm]       = useState({ name:user.name, phone:user.phone, address:user.address, subject:user.subject });
  ...
  return (
    <div style={{ animation:"fadeIn 0.3s ease", maxWidth:640 }}>
      ...
      {/* Profile header */}
      <div style={{ background:"white", borderRadius:20, padding:"28px", border:"1px solid #f1f5f9",
        ...
        <div style={{ display:"flex", alignItems:"center", gap:16, marginBottom:24, paddingBottom:20, borderBottom:"1px solid #f3f4f6" }}>
          <div style={{ width:72, height:72, borderRadius:16, background:"linear-gradient(135deg,#3b82f6,#1d4ed8)",
            display:"flex", alignItems:"center", justifyContent:"center", fontSize:28, fontWeight:800, color:"white" }}>
            {user.name?.[0]}
          </div>
          <div style={{ flex:1 }}>
            <h2 style={{ fontSize:20, fontWeight:800, color:"#1c1917", margin:"0 0 6px" }}>{user.name}</h2>
            <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
              <Badge children={`${user.subject} Teacher`} color="#1d4ed8" bg="#dbeafe"/>
              <Badge children={user.batch||"SpacECE"} color="#d97706" bg="#fef3c7"/>
            </div>
          </div>
          <button onClick={()=>setEditing(!editing)} style={editing ? S.btnOrange : S.primaryBtn}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
          {[
            ...
            <div key={i} style={{ background:"#f9fafb", borderRadius:10, padding:"10px 14px", border:"1px solid #f3f4f6" }}>
              ...
              {editing && r.editable
                ? <input style={{ ...S.input, padding:"5px 8px", fontSize:12, background:"white" }}
                    value={r.val} onChange={e=>setForm({...form,[r.key]:e.target.value})}/>
        ...
        {editing && (
          <button onClick={()=>setEditing(false)} style={{ ...S.primaryBtn, width:"100%", marginTop:16 }}>
            💾 Save Changes
          </button>
        )}
      </div>
  ...
  return (
    <div style={{ animation:"fadeIn 0.3s ease" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
        {notifications.map(n=>(
          <div key={n.id} onClick={()=>setNotifications(prev=>prev.map(p=>p.id===n.id?{...p,read:true}:p))}
            style={{ background:n.read?"white":"#fffbeb", borderRadius:14, padding:"14px 18px",
              border:`1px solid ${n.read?"#f1f5f9":"#fbbf24"}`, display:"flex", alignItems:"center",
              gap:14, cursor:"pointer", boxShadow:"0 1px 4px rgba(0,0,0,0.04)",
              borderLeft:`4px solid ${n.read?"#e5e7eb":"#f59e0b"}` }}>
            <div style={{ width:40, height:40, borderRadius:10, background:n.read?"#f3f4f6":"#fef3c7",
...
export default function TeacherDashboard({ user, onLogout }) {
  ...
  return (
    <div style={{ display:"flex", minHeight:"100vh", background:"#f8fafc", fontFamily:"'Segoe UI','Inter',-apple-system,sans-serif" }}>
      ...
      {/* ── Sidebar ── */}
      <div style={{ width:240, background:"white", borderRight:"1px solid #f1f5f9", display:"flex",
        flexDirection:"column", flexShrink:0, boxShadow:"2px 0 12px rgba(0,0,0,0.04)",
        position:"sticky", top:0, height:"100vh", overflowY:"auto" }}>
        <div style={{ padding:"20px 16px 12px" }}>
          <Logo size={120}/>
          <div style={{ textAlign:"center", padding:"4px 12px", borderRadius:20, fontSize:11, fontWeight:700,
            background:"#dbeafe", color:"#1e40af", border:"1px solid #bfdbfe",
            margin:"6px auto 0", display:"inline-block", width:"fit-content" }}>
            🎓 Teacher Panel
          </div>
        </div>
          ...
          {navItems.map(item=>(
            <button key={item.key} onClick={()=>setActiveTab(item.key)}
              style={{ width:"100%", display:"flex", alignItems:"center", gap:8, padding:"9px 12px",
                ...
                background:activeTab===item.key?"#dbeafe":"transparent",
                color:activeTab===item.key?"#1e40af":"#6b7280",
              ...
              {item.badge>0 && (
                <span style={{ background:"#ef4444", color:"white", borderRadius:20,
                  fontSize:10, fontWeight:800, padding:"1px 7px" }}>{item.badge}</span>
              )}
            </button>
          ))}
        </nav>
        ...
        <div style={{ padding:"12px 16px", borderTop:"1px solid #f1f5f9", display:"flex", alignItems:"center", gap:10 }}>
          <div style={{ width:34, height:34, borderRadius:"50%", background:"linear-gradient(135deg,#3b82f6,#1d4ed8)",
            display:"flex", alignItems:"center", justifyContent:"center", fontSize:14, fontWeight:800, color:"white", flexShrink:0 }}>
            {user.name?.[0]}
          </div>
