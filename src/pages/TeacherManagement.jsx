...
/* ═══════════════════════════════════════════
   ENHANCED TEACHER MANAGEMENT TAB — A2.1 + A2.2
   Drop-in replacement for TeacherManagementTab
═══════════════════════════════════════════ */
...
/* ── Sub-component: Progress Report Modal ── */
    ...
    <Modal title={`📊 Progress Report — ${teacher.name}`} onClose={onClose}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 18 }}>
        {[
          ...
          { icon: "🎓", label: "Avg Score",       val: `${Math.round(modules.filter(m=>m.score).reduce((a,m)=>a+m.score,0)/completed)}%` },
        ].map((s, i) => (
      ...
      {modules.map((m, i) => (
        <div key={i} style={{ marginBottom: 12, padding: "10px 14px", background: m.done ? "#ecfdf5" : "#f9fafb", borderRadius: 10, border: `1px solid ${m.done ? "#86efac" : "#f1f5f9"}` }}>
      ...
      {/* Attendance trend */}
      <div style={{ fontSize: 13, fontWeight: 700, color: "#374151", marginTop: 4, marginBottom: 8 }}>Attendance Rate</div>
...
/* ── Sub-component: Direct Message Modal ── */
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {["in-app", "email", "sms"].map(c => (
            <button key={c} onClick={() => setChannel(c)}
              style={{ flex: 1, padding: "8px", borderRadius: 8, border: `1.5px solid ${channel === c ? "#f59e0b" : "#e5e7eb"}`, background: channel === c ? "#fef3c7" : "white", color: channel === c ? "#92400e" : "#6b7280", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", textTransform: "capitalize" }}>
...
function RejectModal({ teacher, onConfirm, onClose }) {
  ...
  return (
    <Modal title={`✕ Reject Registration — ${teacher.name}`} onClose={onClose}>
      <div style={{ background: "#fef2f2", border: "1px solid #fca5a5", borderRadius: 10, padding: "12px 14px", marginBottom: 16, fontSize: 12, color: "#991b1b" }}>
        ⚠️ The teacher will be notified via email with the rejection reason.
      </div>
      <label style={S.label}>Reason for Rejection *</label>
      <select style={{ ...S.input, marginBottom: 12 }} value={reason} onChange={e => setReason(e.target.value)}>
        <option value="">Select a reason...</option>
        {reasons.map(r => <option key={r}>{r}</option>)}
      </select>
      <label style={S.label}>Additional Notes</label>
      <textarea style={{ ...S.input, height: 80, resize: "none", marginBottom: 20 }} placeholder="Optional additional notes..." />
...
function BlockModal({ teacher, onConfirm, onClose }) {
  ...
  return (
    <Modal title={`🚫 Block Teacher — ${teacher.name}`} onClose={onClose}>
      <div style={{ background: "#fef2f2", border: "1px solid #fca5a5", borderRadius: 10, padding: "12px 14px", marginBottom: 16, fontSize: 12, color: "#991b1b" }}>
        ⚠️ Blocking will suspend the teacher's access. They can be unblocked later.
      </div>
      <label style={S.label}>Reason for Block *</label>
...
/* ── Sub-component: Edit Courses Modal ── */
function EditCoursesModal({ teacher, onSave, onClose, setToast }) {
  const ALL_COURSES = [
  ...
  return (
    <Modal title={`📚 Edit Enrolled Courses — ${teacher.name}`} onClose={onClose}>
      <div style={{ fontSize: 12, color: "#6b7280", marginBottom: 14 }}>
        Select courses to assign or remove for this teacher.
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 20 }}>
        {ALL_COURSES.map(c => (
          <div key={c} onClick={() => toggle(c)}
            style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", borderRadius: 10, cursor: "pointer", border: `1.5px solid ${selected.includes(c) ? "#f59e0b" : "#e5e7eb"}`, background: selected.includes(c) ? "#fef3c7" : "#f9fafb", transition: "all 0.15s" }}>
      ...
      <button onClick={() => onSave(selected)} style={{ ...S.primaryBtn, width: "100%" }}>
        Save Course Assignments ({selected.length} selected)
      </button>
    </Modal>
  );
}

/* ── Sub-component: Issue Certificate Modal ── */
function IssueCertificateModal({ teacher, onClose, setToast }) {
  ...
  return (
    <Modal title={`🏅 Issue Manual Certificate — ${teacher.name}`} onClose={onClose}>
      <div style={{ background: "#fef3c7", border: "1px solid #fbbf24", borderRadius: 10, padding: "10px 14px", marginBottom: 14, fontSize: 12, color: "#92400e" }}>
        🛈 This overrides automatic certificate generation. Use for special cases only.
      </div>
...
function ResetPasswordModal({ teacher, onClose, setToast }) {
  ...
  const sendReset = () => {
    setSent(true);
    setTimeout(() => {
      setToast({ msg: `Password reset OTP sent to ${teacher.email}!`, type: "success" });
      onClose();
    }, 1200);
  };
  ...
  return (
    <Modal title={`🔒 Reset Password — ${teacher.name}`} onClose={onClose}>
      <div style={{ background: "#f0f9ff", border: "1px solid #bae6fd", borderRadius: 10, padding: "12px 14px", marginBottom: 16, fontSize: 12, color: "#0369a1" }}>
        An OTP will be sent to the teacher's registered email address. They will use it to set a new password.
      </div>
      ...
      <button onClick={sendReset} disabled={sent}
        style={{ ...S.primaryBtn, width: "100%", opacity: sent ? 0.7 : 1 }}>
...
/* ── Sub-component: Bulk Notification Modal ── */
function BulkNotifyModal({ teachers, onClose, setToast }) {
  const [subject, setSubject] = useState("");
  const [body, setBody] = useState("");
  const send = () => {
    if (!subject || !body) { setToast({ msg: "Fill subject and message.", type: "error" }); return; }
    setToast({ msg: `Notification sent to ${teachers.length} selected teachers!`, type: "success" });
    onClose();
  };
  return (
    <Modal title={`🔔 Bulk Notify (${teachers.length} teachers)`} onClose={onClose}>
...
/* ══════════════════════════════════════════════════════
   FULL PROFILE VIEW (A2.2 — View Full Profile)
══════════════════════════════════════════════════════ */
function TeacherProfileView({ teacher, onBack, onUpdate, setToast, allCourses }) {
  ...
  return (
    <div style={{ animation: "fadeIn 0.3s ease" }}>
      {/* Modals */}
      {showReject   && <RejectModal   teacher={teacher} onClose={() => setShowReject(false)}  setToast={setToast} onConfirm={reason => { onUpdate({ ...teacher, status: "rejected", rejectReason: reason }); setShowReject(false); setToast({ msg: "Teacher rejected & notified.", type: "error" }); }}/>}
      {showBlock    && <BlockModal    teacher={teacher} onClose={() => setShowBlock(false)}   setToast={setToast} onConfirm={reason => { onUpdate({ ...teacher, status: "rejected", blockReason: reason }); setShowBlock(false); setToast({ msg: "Teacher blocked.", type: "error" }); }}/>}
      ...
      {showCourses  && (
        <EditCoursesModal teacher={teacher} onClose={() => setShowCourses(false)} setToast={setToast}
          onSave={selected => {
            onUpdate({ ...teacher, course: selected[0] || "" });
            setToast({ msg: "Course assignments updated!", type: "success" });
        ...
        <div style={{ display: "flex", alignItems: "flex-start", gap: 20, marginBottom: 24, paddingBottom: 20, borderBottom: "1px solid #f3f4f6" }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 8 }}>
              ...
              <span style={{ padding: "3px 10px", borderRadius: 20, fontSize: 11, fontWeight: 700, color: "#d97706", background: "#fef3c7", border: "1px solid #fbbf2440" }}>{teacher.subject}</span>
              {teacher.batch && <span style={{ padding: "3px 10px", borderRadius: 20, fontSize: 11, fontWeight: 700, color: "#7c3aed", background: "#ede9fe", border: "1px solid #c4b5fd40" }}>{teacher.batch}</span>}
          ...
          {/* Action Buttons */}
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "flex-end", maxWidth: 300 }}>
            {isPending && (
              <>
                <button onClick={() => { onUpdate({ ...teacher, status: "approved" }); setToast({ msg: "Teacher approved!", type: "success" }); }}
                  style={S.btnGreen}>✓ Approve</button>
                <button onClick={() => setShowReject(true)} style={S.btnRed}>✕ Reject</button>
              </>
            )}
            {isApproved && (
              <button onClick={() => setShowBlock(true)} style={S.btnOrange}>🚫 Block</button>
            )}
            {isBlocked && (
              <button onClick={() => { onUpdate({ ...teacher, status: "approved" }); setToast({ msg: "Teacher unblocked & access restored!", type: "success" }); }}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 10 }}>
          {quickActions.map((a, i) => (
            <button key={i} onClick={a.onClick}
              style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, padding: "12px 8px", border: `1px solid ${a.color}30`, borderRadius: 12, background: a.bg, cursor: "pointer", fontFamily: "inherit", transition: "all 0.2s" }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        {["overview", "documents", "activity"].map(tab => (
          <button key={tab} onClick={() => setActiveSection(tab)}
            style={{ padding: "8px 18px", borderRadius: 8, border: `1.5px solid ${activeSection === tab ? "#f59e0b" : "#e5e7eb"}`, background: activeSection === tab ? "#fef3c7" : "white", color: activeSection === tab ? "#92400e" : "#6b7280", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", textTransform: "capitalize" }}>
            ...
            <SectionCard title="📊 Attendance & Performance">
              {teacher.status === "approved" ? (
                <>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                    {[
                      { label: "Revenue Generated", val: `₹${(teacher.revenue / 1000).toFixed(1)}k`, icon: "💰", color: "#10b981", bg: "#d1fae5" },
                      { label: "Sessions Attended", val: `${teacher.classes * 3}`, icon: "📹", color: "#3b82f6", bg: "#dbeafe" },
                    ].map((s, i) => (
                      <div key={i} style={{ background: s.bg, borderRadius: 10, padding: "12px", border: `1px solid ${s.color}30`, textAlign: "center" }}>
                  ...
                  {/* Rejection/Block reason badge */}
                  {teacher.rejectReason && (
                    <div style={{ marginTop: 12, background: "#fee2e2", borderRadius: 8, padding: "8px 12px", fontSize: 12, color: "#991b1b" }}>
                      ✕ Rejected: {teacher.rejectReason}
                    </div>
                  )}
                  {teacher.blockReason && (
                    <div style={{ marginTop: 12, background: "#fef3c7", borderRadius: 8, padding: "8px 12px", fontSize: 12, color: "#92400e" }}>
                      🚫 Blocked: {teacher.blockReason}
                    </div>
                  )}
                </>
              ) : (
        ...
        <SectionCard title="📄 Uploaded Documents">
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
            {[
              ...
              { name: "Experience Letter",     status: teacher.experience === "Fresher" ? "na" : "pending", icon: "💼" },
            ].map((doc, i) => (
              <div key={i} style={{ background: "#f9fafb", borderRadius: 12, padding: "14px", border: "1px solid #f1f5f9", textAlign: "center" }}>
                ...
                <StatusBadge status={doc.status === "na" ? "draft" : doc.status} />
                {doc.status !== "na" && (
                  <div style={{ marginTop: 8 }}>
                    <button style={S.tblBtn}>View</button>
                  </div>
                )}
              </div>
            ))}
      ...
      {activeSection === "activity" && (
        <SectionCard title="🕓 Activity Log">
          {[
            ...
            { action: "Profile verified by admin",        time: "—",            icon: "✅", type: "success"  },
            ...
            <div key={i} style={{ display: "flex", gap: 12, padding: "10px 0", borderBottom: "1px solid #f3f4f6" }}>
              <div style={{ width: 32, height: 32, borderRadius: 8, background: a.type === "success" ? "#d1fae5" : "#dbeafe", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, flexShrink: 0 }}>
                {a.icon}
              </div>
              <div style={{ flex: 1 }}>
...
/* ══════════════════════════════════════════════════════
   MAIN TEACHER MANAGEMENT TAB — A2.1 + A2.2
══════════════════════════════════════════════════════ */
export default function TeacherManagementTab({ teachers, setTeachers, setToast }) {
  ...
  const [newT, setNewT] = useState({
    ...
    qualification:"Graduate", experience:"Fresher", course:"", password:""
  });
  ...
  /* If profile selected, show full profile view */
  if (selected) {
    return (
      <TeacherProfileView
        teacher={selected}
    ...
    <div style={{ animation:"fadeIn 0.3s ease" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20 }}>
        <div>
          ...
          <p style={S.pageSub}>
            {teachers.length} total &nbsp;·&nbsp; {teachers.filter(t=>t.status==="approved").length} active &nbsp;·&nbsp; {teachers.filter(t=>t.status==="pending").length} pending
          </p>
        </div>
      ...
      <div style={{ background:"white", borderRadius:16, padding:"16px 20px", border:"1px solid #f1f5f9", boxShadow:"0 2px 8px rgba(0,0,0,0.04)", marginBottom:16 }}>
        ...
        <div style={{ display:"flex", gap:10, flexWrap:"wrap", marginBottom:12 }}>
          <div style={{ flex:1, minWidth:200 }}>
            <SearchBar value={search} onChange={setSearch} placeholder="Search by name, email, mobile, subject..." />
          </div>
          <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
            {["all","approved","pending","rejected"].map(f => (
              <button key={f} onClick={() => setStatusFilter(f)}
                style={{ padding:"8px 14px", borderRadius:8, border:`1.5px solid ${statusFilter===f?"#f59e0b":"#e5e7eb"}`, background:statusFilter===f?"#fef3c7":"white", color:statusFilter===f?"#92400e":"#6b7280", fontSize:12, fontWeight:600, cursor:"pointer", fontFamily:"inherit", textTransform:"capitalize" }}>
                {f==="all" ? "All Status" : f==="approved" ? "✅ Active" : f==="pending" ? "⏳ Pending" : "🚫 Blocked"}
                {f==="pending" && teachers.filter(t=>t.status==="pending").length > 0
      ...
      {selectedIds.length > 0 && (
        <div style={{ background:"#fef3c7", border:"1px solid #fbbf24", borderRadius:12, padding:"12px 16px", marginBottom:14, display:"flex", alignItems:"center", gap:12, flexWrap:"wrap" }}>
          ...
          <button onClick={() => setBulkModal(true)} style={{ ...S.tblBtn, color:"#3b82f6", borderColor:"#93c5fd" }}>🔔 Send Notification</button>
          <button onClick={() => exportCSV(teachers.filter(t=>selectedIds.includes(t.id)), "selected_teachers.csv")}
            style={S.exportBtn}>⬇ Export Selected CSV</button>
          <button onClick={() => setSelectedIds([])} style={S.tblBtn}>✕ Deselect All</button>
        </div>
      )}
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
        ...
        <button onClick={() => exportCSV(filtered, "teachers_export.csv")} style={S.exportBtn}>
          ⬇ Export CSV
        </button>
      </div>
      ...
      <div style={{ background:"white", borderRadius:16, border:"1px solid #f1f5f9", overflow:"hidden", boxShadow:"0 2px 8px rgba(0,0,0,0.04)" }}>
        <table style={{ width:"100%", borderCollapse:"collapse" }}>
          ...
          <tbody>
            {filtered.map((t, i) => (
              <tr key={t.id} style={{ borderBottom:"1px solid #f9fafb", background:selectedIds.includes(t.id)?"#fffbeb":i%2===0?"white":"#fafafa", transition:"background 0.15s" }}>
                {/* Checkbox */}
                ...
                <td style={{ padding:"12px 14px" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                    <div style={{ width:36, height:36, borderRadius:10, background:"linear-gradient(135deg,#f59e0b,#d97706)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:15, fontWeight:800, color:"white", flexShrink:0 }}>
                      {t.name[0]}
                    </div>
                    <div>
                ...
                <td style={{ padding:"12px 14px", fontSize:12, color:"#374151", maxWidth:160 }}>
                  <span style={{ display:"block", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
                    {t.course || <span style={{ color:"#d1d5db" }}>—</span>}
                  </span>
                </td>
                {/* Batch */}
                ...
                <td style={{ padding:"12px 14px" }}>
                  <div style={{ display:"flex", gap:5, flexWrap:"wrap" }}>
                    <button onClick={() => setSelected(t)} style={{ ...S.tblBtn, color:"#3b82f6", borderColor:"#93c5fd" }}>
                      👁 View
                    </button>
                    {t.status==="pending" && (
                      <>
                    ...
                    {t.status==="approved" && (
                      <button onClick={() => setSelected(t)} style={S.tblBtn}>⋯</button>
                    )}
                  </div>
                </td>
              </tr>
          ...
          <form onSubmit={handleAdd}>
            ...
            <div style={{ marginTop:12 }}>
              ...
              <div style={{ position:"relative" }}>
                ...
                <input style={{ ...S.input, paddingLeft:32 }} type="password" value={newT.password}
                  onChange={e => setNewT({ ...newT, password:e.target.value })} placeholder="Set a password" />
              </div>
            </div>
