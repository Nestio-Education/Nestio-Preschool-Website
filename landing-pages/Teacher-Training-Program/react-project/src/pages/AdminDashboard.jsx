...
export default function AdminDashboard({ user, onLogout }) {
  ...
  /* ── Map backend assignment shape to review shape (from current file) ── */
  ...
  /* ── Nav items — emoji icons from previous file ── */
  const navItems = [
    ...
    //{ key:"assessments",  label:"Assessment Mgmt",    icon:"🧠" },
    //{ key:"certificates", label:"Certificates",       icon:"🏅" },
    //{ key:"sessions",     label:"Live Sessions",      icon:"📹" },
  ];

  /* ── persistTeachers — backend version (from current file) ── */
  ...
  /* ── renderContent (backend version from current file) ── */
  const renderContent = () => {
    switch (activeTab) {
  ...
  /* ── Fetch from backend on mount (from current file) ── */
  useEffect(() => {
    let ignore = false;
